...
obs_counts_raw$KO <- rownames(obs_counts_raw)
...
raw_amr <- obs_counts_raw %>%
  filter(KO %in% eggnog_amr$KO)
write_tsv(raw_amr, "outputs/functional_tables/antibiotic_resistance/observational_amr_raw_counts.tsv")
...
raw_vitamin <- obs_counts_raw %>%
  filter(KO %in% eggnog_vitamin$KO)
write_tsv(raw_vitamin, "outputs/functional_tables/vitamins//observational_vitamins_raw_counts.tsv")
...
raw_metabolism <- obs_counts_raw %>%
  filter(KO %in% eggnog_metabolism$KO)
write_tsv(raw_metabolism, "outputs/functional_tables/metabolism/observational_ko01100_raw_counts.tsv")

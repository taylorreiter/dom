...
# summarize them to per-sample counts. Then, these are filtered to orthologs
...
all_counts <- tximport(files = all$filename, type = "salmon", tx2gene = txi_kegg)
all_counts_raw <- all_counts$counts
all_counts_raw <- as.data.frame(all_counts_raw)
colnames(all_counts_raw) <- info$sample
all_counts_raw$KO <- rownames(all_counts_raw)
...
kegg_pathwas <- read_tsv("outputs/functional_tables/ko_to_pathway_map.tsv")
...
raw_amr <- all_counts_raw %>%
  filter(KO %in% eggnog_amr$KO)
write_tsv(raw_amr, "outputs/functional_tables/antibiotic_resistance/all_amr_raw_counts.tsv")
...
raw_vitamin <- all_counts_raw %>%
  filter(KO %in% eggnog_vitamin$KO)
write_tsv(raw_vitamin, "outputs/functional_tables/vitamins/all_vitamins_raw_counts.tsv")
...
raw_metabolism <- all_counts_raw %>%
  filter(KO %in% eggnog_metabolism$KO)
write_tsv(raw_metabolism, "outputs/functional_tables/metabolism/all_ko01100_raw_counts.tsv")
...
raw_starch <- all_counts_raw %>%
  as.data.frame() %>%
  filter(KO %in% eggnog_starch$KO)
write_tsv(raw_starch, "outputs/functional_tables/starch_and_sucrose_metabolism/all_starch_and_sucrose_raw_counts.tsv")
